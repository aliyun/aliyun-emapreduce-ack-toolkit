...
public final class AppUrls {
    ...
    /**
     * Spark History Server URL for an application, or null when the history server is not
     * configured or the appId is unknown. Works for both {@code application_*} (YARN) and {@code
     * spark-*} (K8s).
     */
    public static String applicationUrl(String historyServerUrl, String appId) {
        if (historyServerUrl == null
                || historyServerUrl.isEmpty()
                || appId == null
                || appId.isEmpty()) {
            return null;
        }
    ...
    /**
     * A clickable, live Spark Web UI URL for a running batch from its appUrl, or null while appUrl
     * is not yet available.
     *
     * <ul>
     *   <li>K8s: appUrl is the driver Service DNS ({@code http://...svc[.cluster.local]:port}),
     *       only reachable inside the cluster, so route it through Kyuubi's {@code /engine-ui/}
     *       proxy. Matched on {@code .svc} rather than the UI port, which is configurable.
     *   <li>YARN: appUrl is the RM tracking URL (already reachable) — returned as-is.
     * </ul>
     *
    ...
    public static String sparkUiUrl(String kyuubiServerUrl, String appUrl) {
        ...
        if (kyuubiServerUrl != null
                && !kyuubiServerUrl.isEmpty()
