...
final class BatchMonitor {
    ...
    static int await(KyuubiClient client, Config config, SparkSubmitArgs submitArgs,
        ...
        String appUrl = AppUrls.applicationUrl(config.getSparkHistoryServerUrl(), response.getAppId());
            ...
            try {
                ...
                if (timeoutMillis != null) {
                    ...
                    if (elapsedMillis >= timeoutMillis) {
                        System.err.println("\n[" + Console.timestamp() + "] Job timeout after "
                                + submitArgs.getTimeoutSeconds() + " seconds.");
                ...
                if (status.getState() != null) {
                    ...
                    if (!currentState.equals(lastState)) {
                        ...
                        System.out.println("\n[" + Console.timestamp() + "] [Status] " + lastState + " -> " + currentState
                                + " (elapsed: " + Console.formatDuration(elapsedSec) + ")");
                ...
                // source this is suppressed (the driver pod log carries the real output); it resumes
                // if the stream falls back so the user is never left with no logs.
                boolean hasNewLogs = false;
                if (!streamingActive(driverStreamer)) {
                    try {
                        KyuubiClient.LogResponse logResponse = client.getBatchLogs(batchId, logOffset, Polling.LOG_FETCH_SIZE);
                        if (logResponse.getLogRowSet() != null && !logResponse.getLogRowSet().isEmpty()) {
                            ...
                            while (logResponse.getLogRowSet() != null
                                ...
                                logResponse = client.getBatchLogs(batchId, logOffset, Polling.LOG_FETCH_SIZE);
                                if (logResponse.getLogRowSet() != null && !logResponse.getLogRowSet().isEmpty()) {
                                    for (String logLine : logResponse.getLogRowSet()) {
                                        System.out.println(logLine);
                                    }
                        ...
                        if (consecutiveErrors == 0) {
                            System.err.println("\n[" + Console.timestamp() + "] Warning: Could not fetch logs: "
                                    + logError.getMessage());
                        }
                    }
                }

                // Treat fresh driver log lines as activity so heartbeats stay quiet while streaming.
                boolean recentDriverActivity = streamingActive(driverStreamer)
                        && (System.currentTimeMillis() - driverStreamer.lastActivityMillis())
                ...
                if (status.isFinished()) {
                    ...
                    if (!streamingWasActive) {
                        try {
                            KyuubiClient.LogResponse finalLogs = client.getBatchLogs(batchId, logOffset, Polling.LOG_FETCH_SIZE);
                            while (finalLogs.getLogRowSet() != null && !finalLogs.getLogRowSet().isEmpty()) {
                                ...
                                finalLogs = client.getBatchLogs(batchId, logOffset, Polling.LOG_FETCH_SIZE);
                            }
                        } catch (IOException e) {
                    ...
                    String finalAppUrl = AppUrls.applicationUrl(config.getSparkHistoryServerUrl(), status.getAppId());
                ...
                if (timeSinceLastHeartbeat >= Polling.HEARTBEAT_LOG_INTERVAL_MS) {
                    if (!hasNewLogs && !recentDriverActivity) {
                        ...
                        System.out.println("[" + Console.timestamp() + "] [Heartbeat] Still running... (state: " + lastState
                                + ", elapsed: " + Console.formatDuration(elapsedSec) + ", idle: " + idleMinutes + "m)");
                    }
                ...
                System.err.println("\n[" + Console.timestamp() + "] Interrupted while waiting for job completion.");
                stop(driverStreamer);
                return ExitCode.INTERRUPTED;
            } catch (IOException e) {
                consecutiveErrors++;
                if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                    System.err.println("\n[" + Console.timestamp() + "] Too many consecutive errors fetching status. Exiting.");
                    System.err.println("Last error: " + e.getMessage());
                    stop(driverStreamer);
                    return ExitCode.ERROR;
                } else if (consecutiveErrors == 1) {
                    System.err.println("\n[" + Console.timestamp() + "] Error fetching status: " + e.getMessage());
    ...
    private static void dumpSubmissionLog(KyuubiClient client, String batchId) {
        try {
            ...
            KyuubiClient.LogResponse logs = client.getBatchLogs(batchId, offset, Polling.LOG_FETCH_SIZE);
            ...
            System.err.println("[" + Console.timestamp() + "] Warning: could not fetch the submission log: "
                    + e.getMessage());
        }
    }
}
