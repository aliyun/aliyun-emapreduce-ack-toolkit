...
/**
 * Stages large SQL out of the K8s pod spec. SQL over {@link #THRESHOLD_BYTES} is uploaded and passed
 * as {@code -f <url>} instead of inline {@code -e}, via a dual strategy:
 * <ol>
 *   <li>Kyuubi server-side upload (needs the kyuubi-upload-plugin; no client-side OSS config); then</li>
 *   <li>client-side OSS upload (needs OSS credentials in {@code --conf}) as a fallback.</li>
 * </ol>
 */
final class SqlUploader {
    ...
    /** SQL larger than this (bytes) is uploaded rather than inlined; conservative for CJK + pod overhead. */
    static final int THRESHOLD_BYTES = 10 * 1024;

    static String upload(KyuubiClient client, byte[] sqlBytes,
                         Map<String, String> conf, Config config,
                         Retry.RetryConfig uploadRetryCfg) throws IOException {
        System.err.println("[" + Console.timestamp() + "] SQL content is " + (sqlBytes.length / 1024)
                + " KB (threshold: " + (THRESHOLD_BYTES / 1024) + " KB), uploading...");
        ...
        try {
            ...
            System.err.println("[" + Console.timestamp() + "] SQL uploaded via Kyuubi server: " + uri);
            return uri;
        } catch (IOException e) {
            ...
            if (pluginNotAvailable) {
                System.err.println("[" + Console.timestamp() + "] Kyuubi upload plugin not available, "
    ...
    private static String uploadToOss(byte[] sqlBytes, Map<String, String> conf,
        ...
        if (accessKeyId == null || accessKeySecret == null || endpoint == null || uploadPath == null) {
            ...
            throw new IOException("SQL content is " + (sqlBytes.length / 1024) + " KB, exceeds "
                    + (THRESHOLD_BYTES / 1024) + " KB threshold. "
        ...
        System.out.println("[" + Console.timestamp() + "] SQL content is " + (sqlBytes.length / 1024)
                + " KB, uploading to OSS...");
